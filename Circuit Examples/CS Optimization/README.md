...
This circuit example shows various optimizations of a common-source (CS) gain stage using the inversion coefficient. In particular, it shows how to optimize the CS gain stage for a given gain-bandwidth product and DC gain. It demonstrates that there is a minimum bias current for achieving a given gain-bandwidth product and DC gain when we account for the effect of capacitive self-loading. The design examples are performed in the [Quarto Notebook](CS_optimization.qmd) and validated with ngspice simulations. The results are presented in the [pdf report](CS_optimization.pdf).
