...
The design and verification is performed in the [Quarto notebook](Miller_OTA.qmd). A design example is given in the [pdf file](Miller_OTA.pdf). An Excel file listing the size and all the variables (namely drain current, small-signal parameters, noise parameters and parasitic capacitances) of each transistor of the circuit.
