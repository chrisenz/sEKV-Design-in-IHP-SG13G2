...
\begin{equation}
 C_2 = 2(C_{GS2} + C_{GB2})
\end{equation}
Assuming M~2a~-M~2b~ are biased in saturation, the gate-to-source capacitance $C_{GS2}$ is given by
\begin{equation}
...
From @tbl-ng_op1 and @tbl-ng_bias, we see that all transistors have a $V_{DS}$ voltage larger than $200\,mV$ and are biased in saturation. Additionnally we see that all the saturation voltages are summing up to `{python} f'{VDsat/1e-3:.0f}'` $mV$ leaving a maximum output voltage swing equal to $V_{out,swing} =$ `{python} f'{Voutswing:.3f}'` $V$.
...
plt.plot([Vidmin/1e-6,0],[Voutq0,Voutq0],'k--', linewidth=lw)
plt.plot([0,0],[0.692,Voutq0],'k--', linewidth=lw)
plt.plot([Vidmin/1e-6,Vos/1e-6],[Voutq,Voutq],'k--', linewidth=lw)
plt.plot([Vos/1e-6,Vos/1e-6],[0.692,Voutq],'k--', linewidth=lw)
