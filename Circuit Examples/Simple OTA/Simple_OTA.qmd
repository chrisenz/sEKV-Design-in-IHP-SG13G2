...
subtitle: For IHP 130nm Process (Version 2)
...
The small-signal schematic corresponding to the schematics of @fig-simple_ota_schematic are shown in @fig-ssc.

![Small-signal schematic of the simple OTA.](Figures/Small-signal_schematics.png){#fig-ssc}

In the case M~1a~-M~1b~ are in the common substrate (left figure), we can recognize the source transconductances in @fig-ssc which are controlled by the common voltage at the source $\Delta V_1$.
...
\end{equation}
where $\tau_2 = 1/\omega_2 \triangleq C_2/G_{m2}$ is the time constant introduced by the current mirror M~2a~-M~2b~ due to the parasitic capacitance $C_2$ at node 2. Note that \eqref{eqn:ymd} is valid for both M~1a~-M~1b~ in a separate well and in a common substrate. This is due to the fact that in differential mode and assuming a perfect matching, the common source node 1 does not change and can be considered as an ac ground. This means that $\Delta V_1=0$ and hence the two small-signal circuits on the left and right of @fig-simplified_ssc become identical.
...
\begin{equation}\label{eqn:gni_def}
  G_{ni}(f)=\gamma_{ni} \cdot G_{mi} + G_{mi}^2 \cdot \frac{\rho_i}{W_i\,L_i\,f},
...
::: {.callout-warning}
An important limitation of the IHP SC13G2 BiCMOS technology @bib:ihp:2025 is the high output conductance and hence the limited intrinsic gain of nMOS transistors. Because of this, the specification on the DC gain, that was initially set to 40 dB, has been downgraded to 30 dB.
:::
...
This results in $C_{GS2} =$ `{python} f'{CGS2/1e-15:.2f}'` $fF$, $C_{GB2} =$ `{python} f'{CGB2/1e-15:.2f}'` $fF$, $C_2 =$ `{python} f'{C2/1e-15:.2f}'` $fF$ and finally $f_{p2} =$ `{python} f'{fp2/1e6:.3f}'` $MHz$, which is much higher than the specified $GBW$.
...
Running the optimizer results in an inversion coefficient $IC_2 =$ `{python} f'{IC2:.0f}'`, a transconductance $G_{m2}  =$ `{python} f'{Gm2/1e-6:.3f}'` $\mu A/V$, a transconductance ratio $G_{m1}/G_{m2}  =$ `{python} f'{Gm1/Gm2:.3f}'`, an aspect ratio $W_2/L_2 =$ `{python} f'{WoverL2:.3f}'` and finally a width and length $W_2 =$ `{python} f'{W2/1e-9:.0f}'` $nm$ and $L_2 =$ `{python} f'{L2/1e-6:.2f}'` $\mu m$. As expected, the non-domiant pole is $f_{p2} =$ `{python} f'{fp2/1e6:.3f}'` $MHz$ corresponding to the target ratio $f_{p2}/GBW =$ `{python} f'{fp2/GBWspec:.0f}'`.
...
From @tbl-flicker, we see that M~2a~-M~2b~ contribute `{python} f'{etafl:.3f}'` times more than M~1a~-M~1b~. This is coming from the fact that $W_1\,L_1$ is `{python} f'{W1*L1/(W2*L2):.0f}'` times larger than $W_2\,L_2$ and that $\rho_p$ is  `{python} f'{rhop/rhon:.1f}'` times larger than $\rho_n$. The flicker noise will therefore be dominated by M~2a~-M~2b~.
...
From @fig-ng_noise, we see that the simulated white noise is slightly larger than the theroretical value. This is probably due to the higher values of the thermal noise excess factors of pMOS transistors as shown in @tbl-ng_op6 compared to the theroretical values given in @tbl-thermal. On the ther hand, the simulated flicker noise perfectly matches the theoretical estimation. The contributions of M~1a~-M~1b~ and M~2a~-M~2b~ to the input-referred white noise PSD are detailed in @fig-ng_white_noise_contributions and compared to the theoretical white noise.
...
textstr = '\n'.join((
  ...
  f'$f_k =$ {fkthe/1e3:.1f} kHz (theory)'))
#    f'$f_k =$ {fksim/1e3:.1f} kHz (sim.)'))
plt.text(1.05, 0.05, textstr, ha='left', va='bottom', size=9, transform=plt.gca().transAxes,
         bbox={'facecolor':'w', 'alpha':1.0, 'pad':5})
plt.show()
```
...
plt.semilogx(fthenz,SnindBvthe,'m--', label='Total (theory)')
plt.xlim(fmin,fmax)
plt.xticks([1e0,1e1,1e2,1e3,1e4,1e5,1e6,1e7,1e8])
plt.xlabel('Frequency [Hz]')
plt.ylim(-150,-100)
...
## Input common-mode voltage range
We can check the input common-mode voltage range by connecting the OTA as a voltage follower and sweeping the positive input. As shown in @fig-ng_cm_range, the output follows the input voltage up to about $0.7\,V$. So the input common-mode voltage range is about $0.7\,V$.
...
plt.plot([0.7,0.7],[0,0.7],'k--', linewidth=lw)
